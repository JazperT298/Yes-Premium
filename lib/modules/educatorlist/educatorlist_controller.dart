...
class EducatorListController extends GetxController {
  ...
  RxList<EducatorsData> userSearchList = <EducatorsData>[].obs;
  int counter = 1;
  Timer? timer;
  @override
  void onInit() {
  ...
  void searchEducator(schoolID, searchKeyword) async {
    try {
      List result = await EducatorAoi.searchEducator(schoolID, searchKeyword);
      educatorsDataList.clear();
      if (!isLoading.value) isLoading(true);
      for (var i = 0; i < result.length; i++) {
        Map mapping = {
