...
class Educators {
  List<EducatorsData>? data;
  String? message;
  String? status;

  Educators({this.data, this.message, this.status});

  Educators.fromJson(Map<String, dynamic> json) {
    if (json['Data'] != null) {
      data = <EducatorsData>[];
      json['Data'].forEach((v) {
        data!.add(new EducatorsData.fromJson(v));
      });
    }
...
EducatorsData eucatorsdataFromJson(String str) =>
    EducatorsData.fromJson(json.decode(str));

String educatorsdataToJson(EducatorsData data) => json.encode(data.toJson());
...
class EducatorsData {
  ...
  EducatorsData.fromJson(Map<String, dynamic> json) {
    userID = json['User_ID'];
